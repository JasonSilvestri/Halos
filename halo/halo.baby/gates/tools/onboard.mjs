...
async function run(seedArg,opts){
  ...
  const locale = (opts.locale || seed.envelope?.onboarding?.locale || "en").toLowerCase();
  const i18nRel = (seed.envelope?.onboarding?.loader || "gates/i18n/onboarding.en.json").replace(/\.en\./, `.${locale}.`);
  ...
  if (!seed.envelope?.onboarding?.enabled || script.length === 0) {console.log("[INFO] Onboarding disabled or empty. Nothing to do.");return;}
